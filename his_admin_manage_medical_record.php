<?php
  ...
  if (isset($_GET['delete_mdr_number'])) {
    // Validasi input untuk memastikan hanya menerima format string yang aman (alphanumeric)
    if (preg_match('/^[a-zA-Z0-9]+$/', $_GET['delete_mdr_number'])) {
        $id = $_GET['delete_mdr_number']; // Ambil nilai sebagai string
        
        // Query DELETE dengan bind parameter
        ...
        $err = "Invalid medical record number format.";
    }
} else {
    $err = "No medical record number provided.";
}
?>
